...
//Helper function for this.courses.map
        let courseHelperVar = course;
        let termHelperVar = term;
        
        for(let i = 0; i < this.courses.length; i++) {
          let objInarray = this.courses[i];
          let termsobjInarray.terms[term]
          if(objInarray.name === course && objInarray.terms[terms]) {
            for (let i = 0; i < objInarray.terms[terms].length)
          }
